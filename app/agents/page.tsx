...
export default function AgentsPage() {
        ...
        try {
            ...
            if (contentType.includes("text/event-stream")) {
                ...
                if (data.agents && Array.isArray(data.agents)) {
                    const mapped: AgentResult[] = data.agents.map((a: { id: string; name: string; emoji: string; status: string; result: string; durationMs: number; error?: string }) => ({
                        id: a.id as AgentId,
                        name: a.name,
                        emoji: a.emoji,
    ...
    const parseSecurityFindings = (text: string): Array<{ severity: string; issue: string; line: string; recommendation: string }> => {
        ...
        const severityRegex = /\*?\*?\[?(CRITICAL|HIGH|MEDIUM|LOW|INFO)\]?\*?\*?\s*[:\-—]?\s*(.+)/gi;
        let match;
